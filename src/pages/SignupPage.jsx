...
export default function SignupPage() {
  ...
  const signUp = async () => {
    if (auth_id === "" || password === "" || username === "" || email === "") {
      // 모든 입력사항 입력 요구
    ...
    if (password !== confirmPassword) {
      // 비밀번호와 비밀번호 확인 타이핑 일치 여부 확인
      return alert("비밀번호와 비밀번호 확인이 다릅니다.");
    }

    try {
      const response = await instance.post("/users", {
        // instance로 url 중복 입력 방지, 프록시에 서버 주소 /api로 변환되어 있고 이를 instance로 대체시켜 axios 치지 않아도 됨.
        auth_id: auth_id,
        password: password,
        username: username,
        email: email,
  ...
  return (
    <div className="login-form">
      <div className="form-group">
        ...
        />
        <button id="check" className="btn" onClick={idCheck}>
          아이디 중복 확인
        </button>
